...
function ENT:SpawnFunction( ply, tr )
    --If it didn't hit something...
 	...
 	local ent = ents.Create( "anom_whirlgig" )
	ent:SetPos( Pos ) 
 	ent:Spawn()
 	ent:Activate() 	 
	return ent 
...
self.DustSize = 120
    ...
    for k, v in pairs( ents.FindInSphere( self.Entity:GetPos(), 300 ) ) do	
		...
		if( v:IsPlayer() and v:IsValid() and v:GetPos( ):Distance( self:GetPos( ) ) <= 370 ) then
		 local dir = v:GetPos() - self:GetPos()
	    self:EmitSound("hgn/stalker/anom/anomaly_mincer_blowout.mp3")
	...
	b:SetKeyValue("DamageType", "0" ) // sonic
	...
	b:Fire("kill", "", 1)
	       -- local effectdata = EffectData()
			...
			effectdata:SetNormal(  tonormlal:Normalize() )  
