...
function ErrorHandler( func, args, ret, errcode )
	...
	RAD.CallHook( "ErrorHandler", func, args, ret, errcode);
	
	local s = func .. "() failed: " .. func .. "(" .. table.concat( args, "," ) .. ") failed with error " .. errcode .. ": " .. RAD.NilFix(ErrorCodes[errcode], "Invalid Error Code");
	
	RAD.DayLog( "errors.txt", s);
	print(s);
