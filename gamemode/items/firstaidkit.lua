...
function ITEM:UseItem(ply)
	...
	for k, v in pairs(ents.FindInSphere(self:GetPos(), 10)) do
		...
		elseif(v != nil and v:IsValid() and v:GetClass() == "prop_ragdoll" and v.isdeathdoll and v.ply:GetNWInt("deathmode") == 1 and v.ply != ply) then
			...
			RAD.SendChat(ply, "*beep* Anabolic Steroids Injected");
			RAD.SendChat(v.ply, "*beep* Anabolic Steroids Injected");
	...
	if(#possible > 1) then
		...
		RAD.SendChat(ply, "*beep* Multiple targets found");
		return;
		
	elseif(#possible == 0) then
	
		RAD.SendChat(ply, "*beep* No target found");
		return;
